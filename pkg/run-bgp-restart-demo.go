...
`
	...
	d := `
kubectl apply -f day2/green-pod-two.yaml
kubectl apply -f day2/red-pod-two.yaml`
...
tmux send-keys -t clients.1 "ip vrf exec red /bin/bash" C-m C-m C-m
tmux send-keys -t clients.1 "curl -sf http://6.6.6.1:5555/hostname" C-m C-m C-m
tmux send-keys -t clients.1 "curl -sf \"http://6.6.6.1:5555/shell?cmd=\"env%7Cgrep%20-i%20node%0A\"\"" C-m C-m C-m
tmux send-keys -t clients.1 "while true;do curl -sf http://6.6.6.1:5555/hostname --connect-timeout 1  -o /dev/null || printf \"%s \" \$(date +%s) ;sleep 1;done" C-m
